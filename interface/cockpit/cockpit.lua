...
-- FU needs custom math here for distance-based fuel cost
    ...
    if distanceMath < 30 then
      cost = 50 + ((config.getParameter("jumpFuelCost") + distanceMath) * 2) -- nearby systems are relatively cheap to travel to
    elseif distanceMath < 200 then
      cost = 70 + ((config.getParameter("jumpFuelCost") + distanceMath) * 4) -- a bit farther out
    elseif distanceMath < 400 then
      cost = 140 + ((config.getParameter("jumpFuelCost") + distanceMath) * 6) -- mid/long range
    else
      cost = 300 + ((config.getParameter("jumpFuelCost") + distanceMath) * 10) -- long range value
    end
